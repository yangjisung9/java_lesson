...
//		Member[]	members = new Member[10];
		...
		System.out.println("2.list의 요소 중 age의 필드 값이 25 이하 인 것만 출력해보기.");	//for
		
		for(int i=0;i<list.size();i++)	{
			if(list.get(i).getAge()<=25)				//list.get(i)로 가져오는 건 객체
		...
		for(int i=0;i<list.size();i++)	{
			...
			//리스트 요소가 여러개의 필드를 갖는 객체일 때, 필드값 비교하는 equals 예제입니다.
			
		System.out.println("4. 참고");
		// indexOf 메소드는 제너릭타입이 String, 기본형 래퍼클래스만 추가적인 기능 구현없이 사용합니다.
		System.out.println("Member 객체로 indexOf 실행 : " + list.indexOf(new Member("나나",20)));	
		//-1, 못찾음 (이유: 참조값이 같은 것을 찾는 것인데 이것을 필드값 비교해서 찾도록 바꾸어 주는 기능 구현이 필요합니다.)
